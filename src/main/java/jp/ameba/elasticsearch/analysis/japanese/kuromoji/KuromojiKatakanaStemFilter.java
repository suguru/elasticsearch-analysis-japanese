...
/**
 * Convert a katakana word to a normalized form by stemming a final
 * KATAKANA-HIRAGANA PROLONGED SOUND MARK (U+30FC) at the end of the word.
 * <p>
 ...
 * quality, but need a well-tuned dictionary. In contract, this filter is simple
 ...
 * {@link KeywordMarkerFilter} or a custom {@link TokenFilter} that sets the
...
public final class KuromojiKatakanaStemFilter extends TokenFilter {
	...
	public KuromojiKatakanaStemFilter(TokenStream in) {
		super(in);
	}
	...
	public final boolean incrementToken() throws IOException {
		if (input.incrementToken()) {
			if (!keywordAtt.isKeyword()) {
				...
				if (length > 3 && buffer[length - 1] == '\u30FC'
						&& isKatakanaString(buffer, length - 1)) {
